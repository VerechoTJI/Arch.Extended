...
public class SparseJaggedArray<T>
{
    ...
    public SparseJaggedArray(int bucketSize, int capacity = 64)
    {
        ...
        {
            SparseBucket<T> bucket = new SparseBucket<T>(_bucketSize, _filler);
    ...
    public SparseJaggedArray(int bucketSize, T filler, int capacity = 64) : this(bucketSize, capacity)
    {
        ...
        for (int i = 0; i < _buckets.Length; i++)
        {
            SparseBucket<T> bucket = new SparseBucket<T>(_bucketSize, filler);
            SetBucket(i, in bucket);
            bucket.Clear();
        }
    }
    ...
    public void Add(int index, in T item)
    {
        ...
        ref SparseBucket<T> bucket = ref GetBucket(bucketIndex);
    ...
    public bool TryGetValue(int index, out T value)
    {
        ...
        ref T item = ref bucket[itemIndex];
    ...
    public void TrimExcess()
    {
        ...
        {
            ref SparseBucket<T> bucket = ref GetBucket(i);
            if (!bucket.IsEmpty)
            {
                break;
            }
    ...
    public void Clear()
    {
        foreach (SparseBucket<T> bucket in _buckets)
        {
            if (bucket.IsEmpty)
            {
                continue;
